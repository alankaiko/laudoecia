...
public class FormPrincipal extends JFrameMdi implements TelaPrincipal
{
    ...
    public boolean sairOk ()
    {
        if (PnlLaudoeCia.foiInstanciado())
        {
            ...
            {
                mostrarMensagemDeErro("Existe um cd\\dvd sendo gravado.\n"
    ...
    public void mostrarComprovanteAtd (Atendimento atd)
    {
        ...
        if (jrv != null)
        {
            JFrameRelatorios frame = new JFrameRelatorios("Comprovante do atendimento: "
                    + atd.getCodigo() + " - " + atd.toString());
            frame.add(jrv);
            frame.setVisible(true);
        }
        setCursorToDefaultCursor();
    ...
    public void mostrarTela (int tela, TelaSolicitante telaSol)
    {
        ...
        Class<? extends JInternalFrame> iframeClass =
                                        ViewFactory.getInstance().getIFrameClass(tela);
        if (iframeClass == null)
        {
            ...
            else
            {
                ...
                {
                    ((DlgCadastro) myJDialog).iniciarBusca(telaSol.getReceptorDeBusca(),
                            telaSol.getParametroDeBusca());
                }
    ...
    private void mostrarIFrame (Class<? extends JInternalFrame> iframeClass,
                                TelaSolicitante telaSol)
    {
        ...
        if (!(telaSol == null || TELA_NULL == telaSol.getCodigo()) &&
            iframe instanceof IFrameCadastro)
        {
    ...
    private void configurarFullScreen (boolean fullScreen)
    {
        ...
        // <editor-fold defaultstate="collapsed" desc="Outra possibilidade, não aceita jdialog">
        ...
        java.awt.EventQueue.invokeLater(new Runnable()
        {
            ...
            public void run ()
            {
                ...
                if (bgColor == 0)
                {
                    try
                    {
                        setBackgroundImage(StaticInfo.getBackgroundImage()
                                , MyJDesktopPane.REDIMENSIONAR);
                    }
