...
public class EditorPanePrinter extends JPanel implements Pageable, Printable {
	...
	public static DlgSaveAsPDF getSavePdfFile() {
		return savePdfFile;
	}
	...
	private void initData(JEditorPane pane, Paper paper, boolean controlaAtdInfo) {
		...
		this.paper = paper;
		this.margins = new Insets(LaudosHTMLUtils.getTopMargin(), LaudosHTMLUtils.getLeftMargin(),
				LaudosHTMLUtils.getBottomMargin(), LaudosHTMLUtils.getRightMargin());
	...
	private void calculatePageInfo() {
		...
		if (startY + pageHeight - headerSize.height - footerSize.height - margins.top - margins.bottom < sourcePane
				.getHeight()) {
			startY = endPageY;
			...
			while (startY + pageHeight - headerSize.height - footerSize.height - margins.top
					- margins.bottom < sourcePane.getHeight()) {
				startY = endPageY;
				endPageY = getEndPageY(startY);
				totalPage++;
			}
		...
		endPageY = getEndPageY(startY);
		int count = 1;
		while (startY + pageHeight - headerSize.height - footerSize.height - margins.top - margins.bottom < sourcePane
				.getHeight()) {
			Shape pageShape = getPageShape(startY, pageWidth - margins.left - margins.right,
					pageHeight - headerSize.height - footerSize.height - margins.top - margins.bottom, sourcePane);
			pages.add(new PagePanel(startY, pageShape, LaudosHTMLUtils.createHeader(controlaAtdInfo && count == 1),
					LaudosHTMLUtils.createFooter(count, totalPage)));
			startY = endPageY;
			endPageY = getEndPageY(startY);
			count++;
		...
		Shape pageShape = getPageShape(startY, pageWidth - margins.left - margins.right,
				pageHeight - headerSize.height - footerSize.height - margins.top - margins.bottom, sourcePane);
		pages.add(new PagePanel(startY, pageShape, LaudosHTMLUtils.createHeader(controlaAtdInfo && totalPage == 1),
				LaudosHTMLUtils.createFooter(totalPage, totalPage)));
				...
				if (resp == JOptionPane.YES_OPTION) {
					...
					try {
						...
						SwingUtils.mostrarAviso(FormPrincipal.getInstance(),
								"Não existe no seu computador um programa para abrir " + "o arquivo.");
					} catch (Exception ex) {
						SwingUtils.mostrarAviso(FormPrincipal.getInstance(),
								"Houve uma falha ao tentar" + " abrir o arquivo:\n" + ex.getMessage());
					}
				}
				tmp.dispose();
			}
	...
	public String savePDFtoWhatsapp(String defaultFileName) {
		String arquivo = null;
	...
	public File savePDFToemail(String defaultFileName) {
		String nomeArquivo = "C:\\temp\\"+ defaultFileName +".pdf";
	...
	private void CriaPasta() {
		...
		if(!arq.exists()) {
			...
			arq.setWritable(Boolean.FALSE, Boolean.TRUE);
			arq.setExecutable(Boolean.FALSE, Boolean.TRUE);
		}
	}
	...
	public void printFromPdf(String jobName, String defaultPrinter, int copies) {
		try {
			...
			new PrintPdf(new FileInputStream(tempPdf), jobName, pageFormat).print(defaultPrinter, copies);
	...
	public void print(String jobName, String defaultPrinter, int copies) {
		try {
			...
			SwingUtils.mostrarAviso(FormPrincipal.getInstance(), "Impressora não localizada, verifique.");
		} catch (Exception ex) {
			SwingUtils.mostrarAviso(FormPrincipal.getInstance(), "Houve uma falha ao imprimir.\n");
		} finally {
			FormPrincipal.getInstance().setCursorToDefaultCursor();
			voltaOriginalStyleSheet();
		}
	...
	public void voltaOriginalStyleSheet() {
		if (sourcePane.getEditorKit() instanceof MyHTMLEditorKit) {
			originalStyleSheet.addRule(
					"div.topico, td { font-family: Arial, Helvetica, sans-serif; font-size: 12pt; } div.topico, table.topico { margin-bottom: 5px; }");
		}
	}
