...
public class DlgMoverImportarImg extends MyJDialog
{
    ...
    {
        super(PnlLaudoeCia.getInstance(), true, new Rectangle(
                new Point(), Toolkit.getDefaultToolkit().getScreenSize()));
    ...
    private void initComponents ()
    {
        ...
        c.gridy = 3;
        c.weighty = 1;
        scrollPane.setVerticalScrollBarPolicy(MyJScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        pnl.add(new PnlComp("Escolha as imagens que serão importadas / movidas",
                false, true).setComponent(scrollPane), c);
        ...
        btnConfirmar.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed (ActionEvent e)
            {
                ...
                else
                {
                    ...
                    {
                        SwingUtils.mostrarMensagemDeErroConhecido(DlgMoverImportarImg.this,
    ...
    private PnlCartao getPnl ()
    {
        ...
        txtDirImagens.getDocument().addDocumentListener(new TxtChangedListener()
        {
            @Override
        ...
        txtAtd.addAutoCompletarListener(new AutoCompletarListener()
        {
            ...
            public void linhaSelecionadaMudou (Object[] oldLinhaSelecionada, Object[] newLinhaSelecionada)
            {
                ...
                {
                    List<ProcDoAtd> procs = new ArrayList<>(
                            buscAtd.getSelectedModel().getProcedimentos());
    ...
    private List<MinhaImagem> obterImagens ()
    {
            ...
            try
            {
                ...
                if (!text.trim().isEmpty() && Files.isDirectory(path))
                {
                    Files.walkFileTree(path, new FileVisitor<Path>()
                    {
                        ...
                        {
                            if (file.getFileName().toString().matches(".+\\.(jpg|jpeg|bmp|png|" +
                                    "JPG|JPEG|PNG|BMP)"))
                            {
                                resposta.add(new MinhaImagem()
                                {
                                    ...
                                    {
                                        return new Imagem(conversor.convertReverse(file.toFile()));
                                    }
        ...
        else
        {
            ...
            for (final Imagem imagen : imagens)
            {
                resposta.add(new MinhaImagem()
                {
                    @Override
    ...
    private void atualizarPreview (byte[] imagem)
    {
        ...
        else
        {
            ...
            Dimension fitSize = ImageUtils.getFitSize(lblImagemPreview.getSize(), aspectRatio);
            lblImagemPreview.setIcon(new ImageIcon(ImageUtils.createResizedCopy(
    ...
    private Component criarPnlDeChk ()
    {
        ...
        for (int linha = 0; linha < numLinhas; linha++)
        {
            ...
            for (int coluna = 0; coluna < 3; coluna++)
            {
                ...
                {
                    c.gridx = coluna;
                    pnl.add(new PnlComEspaco(
                            chksList.get(indexImg), 4, 4, 0, 0), c);
                    indexImg++;
                }
            }
        }
        ...
        java.awt.EventQueue.invokeLater(new Runnable()
        {
            ...
            public void run ()
            {
                if (chksList.size() > 0)
                {
                    ...
                    atualizarPreview(mapaDeImg.get(chksList.get(0)).getImagem().getImagem());
                }
            }
