...
public class PnlLaudo extends MyJPanel
{
    ...
    private void atualizarDadosDoPreview (int indexCdl)
    {
        ...
        try
        {
            if (indexCdl == -1)
            {
                txtHtmlPreview.setCampoDoLaudo(
                        activeSw.getLaudo().getPlanilha());
            }
            else
            {
                txtHtmlPreview.setCampoDoLaudo(
                        activeSw.getLaudo().getPlanilha().getCampo(indexCdl));
            }
        }
        catch (Exception ex)
        {
    ...
    {
        if (buscadorTxtPreview.planilhaFoiEditada()
            && !buscadorTxtPreview.isImpLaudoSelected())
        {
    ...
    private void actionLblValorPadrao ()
    {
        if (lblValorPadrao.isEnabled() && SwingUtils.obterConfirmacaoDoUsuario(
                ...
                + "Tem certeza que deseja continuar?", this) == JOptionPane.YES_OPTION)
        {
...
//        }
    ...
    private class DlgPersonalizarCdl extends MyJDialog
    {
...

        {
            ...
            editor = editor.withAddTextosPadrao(activeSw.getLaudo().
                    getProcDoAtd().getProcMedico());
        ...
        private void initView ()
        {
            ...
            c.insets.right = 0;
            c.insets.bottom = 10;
            contentPane.add(new LblNegrito("<html><font color=\"ff0000\">"
                                           + "Atenção: Quando é usada a opção \"EDITAR\" novas "
                                           + "mudanças no laudo<br> só podem ser feitas "
                                           + "diretamente nessa janela.</font></html>",
