...
public abstract class PnlLayoutImagens extends MyJPanel
{
    ...
    {
        MyItemLayout item = new MyItemLayout(mediador, width, heigth, this, indexItem);
        itens.add(item);
        return item;
    }
    ...
    protected String gerarImgHtmlTag (int indexImg, int paginas, int qtdFotos)
    {
        ...
        return "<img src=\"" + String.valueOf(img) + "\" width=\"" + String.valueOf(item.getLargura()) + "\" height=\""
                + String.valueOf(item.getAltura()) + "\"><font size=\"1\"><p align='center'>" +
                "Imagem " + ++labelImagem + " </p></font> </img>";
    }
    ...
    public void preencherAutomaticamente (List<byte[]> imagens)
    {
        ...
        int loopEnd = imagens.size() < itens.size() ? imagens.size() : itens.size();
