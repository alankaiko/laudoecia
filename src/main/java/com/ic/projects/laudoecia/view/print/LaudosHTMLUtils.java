...
public class LaudosHTMLUtils
{
    ...
    private static String createAtdInfo ()
    {
        ...
        String solicte = "<font size=\"2\">" + (atd.getSolicitante() == null ? "&nbsp;" : atd.getSolicitante()
                .getNome()) + "</font>";
        ...
        String codigoAtd = StaticInfo.getParametrosDoSistema().isImprimirAtd() ?
                "<td align=\"right\"><font size=\"2\"><b>ATENDIMENTO:"
                        + "&nbsp;" + codigo + "</b></font></td>" :
                "<td></td>";
        ...
        if (StaticInfo.LICENCIADO_LAUDO.equals(StaticInfo.LICENCIADO_LAUDO_VERSAO_TESTES))
        {
            if (atd.getCodigo() > 20)
            {
                SwingUtils.mostrarAviso(FormPrincipal.getInstance(), "Versão de demonstração expirou!\n" +
        ...
        String html = "<div style=\"padding: 3px; border-bottom: 1px solid #000000; " + styleBorderTop + "\">"
                ...
                + "<tr>"
                + "<td width=\"60\"><font size=\"2\"><b>PACIENTE:</b></font></td>"
                + "<td><font size=\"2\"><b>" + paciente + "</b></font></td>"
                + codigoAtd
                + "</tr>"
                + "<tr>"
                + "<td><font size=\"2\">DATA:</font></td>"
                + "<td><font size=\"2\">" + dataAtd + "</font></td>"
                + "<td align=\"right\"><font size=\"2\">D.N.:&nbsp;" + dn + "&nbsp;&nbsp;&nbsp;IDADE:&nbsp;" +
                idadeOss + "&nbsp;&nbsp;&nbsp;&nbsp;</font></td>"
                + "</tr>"
                + getSolCon(solicte, convenio)
                + "</table>"
                + "</div>"
                + "<div align=\"right\" width=\"99%\">" + fraseLicenciado + "</div>"
                + "<div style=\"margin-top: " + mt + "px; margin-bottom: "
                + mb + "px; background: #DDDDDD;\" align=\"center\">"
                + "<font><b>" + titulo + "</b></font>"
                + "</div>";
        return html;
    }
    ...
    private static String getSolCon (String solicte, String convenio)
    {
        ...
        {
            String sol = solicte == null || solicte.isEmpty() ? "" :
                    "Dr(a).&nbsp;" + solicte;

            return "<tr>"
                    + "<td><font size=\"2\">SOLICITANTE:</font></td>"
                    + "<td><font size=\"2\">" + sol + "</font></td>"
                    + "<td align=\"right\"><font size=\"2\">CONVÊNIO:&nbsp;" + convenio + "</font></td>"
                    + "</tr>";
        }
        return "";
    }
    ...
    public static String createAssProf (ProfExecutante prof)
    {
        ...
        String cons = prof.getConselho().getSigla() + "-"
                + prof.getUfConselho().getSigla() + " "
        ...
        String html = "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">\n"
                + "<tr><td width=\"30%\">&nbsp;</td>\n"
                + "<td style=\"border-bottom:1px solid #000000; margin-top: " + m + "px;\">"
                + getAss(img, prof)
                + "</td>\n"
                + "<td width=\"30%\">&nbsp;</td></tr>\n"
                + "<tr>\n"
    ...
    private static String getAss (int img, ProfExecutante prof)
    {
        if (img >= 0)
        {
            ...
            //return "<img src=\"" + imgs + "\">";
            return "<img src=\"" + imgs + "\" width=\"" + width
                    + "\" height=\"" + height + "\">";
        }
        return "&nbsp;";
    }
                                                ...
                                                ProfExecutante prof, SubcategoriaCID10 cid)
    {
        ...
        html += "<p align='justify'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + StaticInfo.getLicenciado().getEndereco()
                .getCidade() + ", " + DateUtils.dataExtenso(new Date()) + ".</p><br /><br /></font>";
        html += createAssProf(prof);
        return html;
    }

    public static String createHtmlAtestadoAcomp (String pac, String acomp, String data, String horaDe, String
            horaAte, ProfExecutante prof, SubcategoriaCID10 cid)
    {
        String html = "<font size='3'><p align='center'><b>ATESTADO</b></p><br /><br />";
    ...
    public static String createTabelaPagina (String html, int numPages, boolean vAlignCenter, int subtrair, boolean
            impAtdInfo)
    {
        ...
        String resp = "<table class=\"page\" width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">"
                + "<tr><td height=\"" + String.valueOf(height) + "\" " + valign + ">"
                + html
                + "</td></tr>"
                + "</table>";
        return resp;
    ...
    private static String getHtmlHeader (boolean impAtdInfo)
    {
        ...
        if (paramSist.isImprimeCabecalhoLaudo())
        {
            ...
            if (logomarcaLaudo != null)
            {
                ...
                imgTag = "<div style=\"margin-left: " + String.valueOf(margem) + "px\"><img src=\"" + String.valueOf
                        (img) + "\" width=\"" + width + "\" height=\"" + height + "\"></div>";
            }
            ...
            {
                textoCabecalho = "<div style=\"border-bottom:1px solid #000000;\">"
                        + StringUtils.getBodyContents(
                        paramSist.getTextoCabecalhoLaudo()) + "</div>";
            }
        }
    ...
    private static String getHtmlFooter (int actualPage, int totalPages)
    {
        ...
        {
            licenciadoPaginaTR += "<td align=\"right\"><font size=\"2\"><i>Página " + actualPage + " de " +
                    totalPages + "&nbsp; </i></font></td>";
        }
        else
        {
        ...
        String tamSepRodape = String.valueOf(
                StaticInfo.getParametrosDoSistema().getTamanhoSepRodape());

        //quando estiver marcado para não imprimir rodapé
        if (!paramSist.isImprimeRodapeLaudo())
        {
            String html = "<html><head></head><body>"
                    + "<div style=\"border-top:" + tamSepRodape + "px solid #000000;\">"
                    + "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"1\">";
        ...
        {
            String end = licenciado.getEndereco() == null ?
                    "" : licenciado.getEndereco().toString();
            texto = "<tr>"
                    + "<td colspan=\"2\" align=\"center\"><font size=\"2\">" + end + "</font></td>"
                    + "</tr>";
            String contact = "";
            ...
            texto += "<tr>"
                    + "<td colspan=\"2\" align=\"center\"><font size=\"2\">" + contact + "</font></td>"
                    + "</tr>";
        }
        else
        {
            texto = "<tr><td colspan=\"2\" align=\"center\"><div>" +
                    StringUtils.getBodyContents(paramSist.getTextoRodapeLaudo())
                    + "</div></td></tr>";
        }
        ...
        {
            frase = "<tr><td colspan=\"2\"><div>" + frase + ""
                    + "</div></td></tr>"; //.replace(" ", "&nbsp;")
        }
        else
        {
        ...
        int w = convPixels.convertForward(paramSist.getPaperWidth() - paramSist.getLeftMargin() - paramSist
                .getRightMargin());
    ...
    private static String getCid (SubcategoriaCID10 cid)
    {
        ...
        else
        {
            return "<p align='justify'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                    + "Enquadrando-se no CID " + cid.getCodigo()
                    + " – " + cid.getNome() + ".</p>";
        }
    }
