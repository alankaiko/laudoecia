	...
	public IFramePacientes() {
		...
		DefBuilder builder = new DefBuilder(new TableConfigurator() {
			...
			public String[] columnsTitles() {
				return new String[] { "Prontuário", "Nome", "Nascimento", "Data de cadastro" };
			}

			@Override
			public int[] columnsWidths() {
				return new int[] { 85, 0, 90, 150 };
			}
			...
			public Class<?>[] columnsClasses() {
				return new Class<?>[] { Integer.class, String.class, Date.class, Date.class };
			}

		});

		builder = builder.setTxtBusca(212);
		builder = builder.setCmbCampos(125, "nome", "prontuário", "data nascimento", "data cadastro", "desativados");
		...
		builder.getScroll().setCellRenderer(3, new MyDateRenderer(Constantes.TIMESTAMP, SwingConstants.CENTER));
		...
		pnlCadastro.start(C_BuscaPaciente.class, C_EntidadePaciente.class, PnlPaciente.class, false,
				C_CadastroPaciente.class);
